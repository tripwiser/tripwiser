...
import TravelTipsScreen from './TravelTipsScreen';
...
export default function TemplatesScreen() {
  ...
  // One-time clear AsyncStorage for templates to force reload of new defaults
  React.useEffect(() => {
    (async () => {
      const key = 'zustand:templateStore';
      const cleared = await AsyncStorage.removeItem(key);
      // Optionally, reload the app or screen after clearing
      // You can comment out the next line after first run
  ...
  React.useEffect(() => {
    mixpanel.track('Templates Screen Viewed');
  }, []);
  ...
  const [activeTab, setActiveTab] = useState<'templates' | 'tips'>('templates');
  ...
  const handleUseTemplate = (template: PackingTemplate) => {
    if (template.templateType === 'itinerary') {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      <View style={{ flex: 1 }}>
        ...
        {/* Guide Title at the very top */}
        <View className="bg-white border-b border-gray-100" style={{ paddingTop: insets.top, backgroundColor: theme.card, borderColor: theme.border }}>
          <Text style={{ fontSize: 28, fontWeight: '700', color: theme.text, marginBottom: 8, paddingLeft: 24, paddingTop: 16 }}>Guide</Text>
        </View>
        {/* Top Tab Switcher */}
        <View className="bg-white border-b border-gray-100" style={{ marginBottom: 12, backgroundColor: theme.card, borderColor: theme.border }}>
          <View className="flex-row">
            {[
              ...
              <Pressable
                ...
                style={{ borderColor: activeTab === tab.key ? theme.primary : 'transparent', backgroundColor: theme.card }}
              >
                <Ionicons 
                ...
                )} style={{ color: activeTab === tab.key ? theme.primary : theme.muted }}>
                  {tab.label}
                </Text>
              </Pressable>
            ))}
        ...
        {activeTab === 'templates' ? (
          <>
            {/* Header */}
            <View className="bg-white border-b border-gray-100" style={{ marginTop: 12, paddingTop: 12, paddingBottom: 12, backgroundColor: theme.card, borderColor: theme.border }}>
              <View className="px-6 py-2">
                ...
                {/* Search */}
                <View className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 flex-row items-center mb-4" style={{ backgroundColor: theme.inputBackground, borderColor: theme.inputBorder }}>
                  ...
                  <View className="flex-row">
                    {categories.map((category) => (
                      ...
                      >
                        ...
                        )} style={{ color: selectedCategory === category ? theme.primary : theme.muted }}>
                          {category}
                        </Text>
                      </Pressable>
                    ))}
            ...
            >
              ...
              {currentTier === 'free' && customTemplates.length === 0 && (
                ...
                >
                  ...
                  <Pressable 
                    ...
                    style={{ borderRadius: 12, overflow: 'hidden', alignSelf: 'flex-start' }}
                  >
                    <LinearGradient
                      colors={['#4F46E5', '#6366F1']}
                      start={{ x: 0, y: 0 }}
              ...
              {filteredTemplates.length === 0 && (
                <View className="items-center justify-center py-12" style={{ backgroundColor: theme.background }}>
                  <Text className="text-xl font-bold text-gray-900 mb-2" style={{ color: theme.text }}>No templates found</Text>
                  <Text className="text-gray-600 text-center" style={{ color: theme.muted }}>
                  ...
                  >
                    ...
                    <View className="flex-row space-x-3">
                      ...
                      >
                        ...
                        >
                          <Text style={{ color: 'white', fontWeight: '600', fontSize: 16 }}>Create Template</Text>
                        </LinearGradient>
                      </Pressable>
                    </View>
                  </Animated.View>
